...
MAPR_VERSION_DSR=${MAPR_VERSION_DSR:-"v1.4.1.2"}
...
if [ "$RELEASE" = true ]; then
    ...
    ZEPPELIN_GIT_TAG=${ZEPPELIN_GIT_TAG:-"branch-0.8.2-mapr"}
...
if [ "$BUILD_SPARK" = true ]; then
    echo "Building Spark images with Zeppelin"
    docker_build "spark/Dockerfile" "${SPARK_REPO}:${SPARK_IMAGE_VERSION}"
    [ "$?" -ne 0 ] && BUILD_SUCC=false
fi
...
if [ "$PUSH_IMAGES" = true ] && [ "$BUILD_SUCC" = true ]; then
    ...
    if [ "$BUILD_SPARK" = true ]; then
        docker push "${SPARK_REPO}:${SPARK_IMAGE_VERSION}"
        docker tag "${SPARK_REPO}:${SPARK_IMAGE_VERSION}" "${SPARK_REPO}:latest"
        docker push "${SPARK_REPO}:latest"
    fi
fi
